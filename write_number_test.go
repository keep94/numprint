...
func TestWriteZeroDigits(t *testing.T) {
	assert.Equal(t, "\n", Swrite(newFakeNumberRange(0, 0)))
	assert.Equal(t, "\n", Swrite(newFakeNumberRange(5, 5)))
}

func TestWriteNoOptions(t *testing.T) {
	actual := Swrite(newFakeNumberRange(0, 12))
...
func TestWriteLessThanOneRow(t *testing.T) {
	actual := Swrite(
		newFakeNumberRange(0, 12), DigitsPerRow(12), DigitsPerColumn(0))
...
func TestWriteColumnsShow(t *testing.T) {
	actual := Swrite(newFakeNumberRange(0, 12), DigitsPerColumn(5), DigitsPerRow(0))
	expected := "0  12345 67890 12\n"
	assert.Equal(t, expected, actual)
}
...
func TestWriteRows10Show(t *testing.T) {
	actual := Swrite(
		newFakeNumberRange(0, 110), DigitsPerRow(10), DigitsPerColumn(0))
	expected := `  0  1234567890
 10  1234567890
 20  1234567890
 30  1234567890
...
func TestWriteRows10ColumnsShow(t *testing.T) {
	actual := Swrite(
		newFakeNumberRange(0, 110), DigitsPerRow(10), DigitsPerColumn(10))
	expected := `  0  1234567890
 10  1234567890
 20  1234567890
 30  1234567890
...
func TestWriteRows11ColumnsShow(t *testing.T) {
	actual := Swrite(
		newFakeNumberRange(0, 110), DigitsPerRow(11), DigitsPerColumn(10))
...
func TestWriteRows11ColumnsShow109(t *testing.T) {
	actual := Swrite(
		newFakeNumberRange(0, 109), DigitsPerRow(11), DigitsPerColumn(10))
	expected := ` 0  1234567890 1
11  2345678901 2
22  3456789012 3
33  4567890123 4
...
func TestWriteWithStart(t *testing.T) {
	actual := Swrite(newFakeNumberRange(502, 505))
	expected := "500  ..345\n"
	assert.Equal(t, expected, actual)
}
...
func TestWriteCountBytes(t *testing.T) {
	...
	n, err := Fwrite(w, newFakeNumberRange(0, 10000))
...
func TestWRiteCountBytes2(t *testing.T) {
	...
	n, err := Fwrite(w, newFakeNumberRange(0, 1000))
	assert.Equal(t, 1300, n)
	assert.NoError(t, err)
}

func TestWriteErrorAtAllStages(t *testing.T) {
	number := newFakeNumberRange(0, 10000)
...
func TestWriteErrorAtAllStages2(t *testing.T) {
	number := newFakeNumberRange(0, 1000)

	// Set buffer size to 1 so that we get extra coverage.
	for i := 0; i < 1300; i++ {
		w := &maxBytesWriter{maxBytes: i}
		n, err := Fwrite(w, number, bufferSize(1))
		assert.Equal(t, i, n)
		assert.Error(t, err)
	}
}
