...
func TestPrintNoOptions(t *testing.T) {
	actual := Sprint(newFakeNumber(), UpTo(12))
...
func TestPrintLessThanOneRow(t *testing.T) {
	actual := Sprint(
		newFakeNumber(), UpTo(12), DigitsPerRow(12), DigitsPerColumn(0))
...
func TestPrintColumnsShow(t *testing.T) {
	actual := Sprint(newFakeNumber(), UpTo(12), DigitsPerColumn(5), DigitsPerRow(0))
	expected := `0.12345 67890 12`
	assert.Equal(t, expected, actual)
}
...
func TestPrinterRows10Show(t *testing.T) {
	actual := Sprint(
		newFakeNumber(), UpTo(110), DigitsPerRow(10), DigitsPerColumn(0))
	expected := `   0.1234567890
 10  1234567890
 20  1234567890
 30  1234567890
...
func TestPrinterRows10ColumnsShow(t *testing.T) {
	actual := Sprint(
		newFakeNumber(), UpTo(110), DigitsPerRow(10), DigitsPerColumn(10))
	expected := `   0.1234567890
 10  1234567890
 20  1234567890
 30  1234567890
...
func TestPrinterRows11ColumnsShow(t *testing.T) {
	actual := Sprint(
		newFakeNumber(), UpTo(110), DigitsPerRow(11), DigitsPerColumn(10))
	expected := `  0.1234567890 1
11  2345678901 2
22  3456789012 3
33  4567890123 4
...
func TestPrinterRows11ColumnsShow109(t *testing.T) {
	actual := Sprint(
		newFakeNumber(), UpTo(109), DigitsPerRow(11), DigitsPerColumn(10))
	expected := `  0.1234567890 1
11  2345678901 2
22  3456789012 3
33  4567890123 4
...
func TestPrinterWithStart(t *testing.T) {
	number := newFakeNumberRange(502, 1000)
	actual := Sprint(number, UpTo(505))
	expected := `500  ..345`
	assert.Equal(t, expected, actual)
	assert.Empty(t, Sprint(number, UpTo(502)))
}
...
func TestPrinterNegative(t *testing.T) {
	actual := Sprint(newFakeNumber(), UpTo(-3), DigitsPerRow(10))
...
func TestPrinterCountBytes(t *testing.T) {
	...
	// line feed char.
	n, err := Fprint(w, newFakeNumber(), UpTo(10000))
...
func TestPrinterCountBytes2(t *testing.T) {
	...
	// line feed char.
	n, err := Fprint(w, newFakeNumber(), UpTo(1000))
	assert.Equal(t, 1299, n)
	assert.NoError(t, err)
}

func TestErrorAtAllStages(t *testing.T) {
	number := newFakeNumber()
...
func newFakeNumber() *fakeNumber {
	return &fakeNumber{}
}

func (f fakeNumber) AllInRange(start, end int) iter.Seq2[int, int] {
...
func (f fakeNumber) BackwardInRange(start, end int) iter.Seq2[int, int] {
	return func(yield func(int, int) bool) {
...
type fakeNumberRange struct {
	Start int
	End   int
	fake  fakeNumber
}

func newFakeNumberRange(start, end int) *fakeNumberRange {
	return &fakeNumberRange{Start: start, End: end}
}

func (f *fakeNumberRange) AllInRange(start, end int) iter.Seq2[int, int] {
	return f.fake.AllInRange(max(start, f.Start), min(end, f.End))
}

func (f *fakeNumberRange) All() iter.Seq2[int, int] {
	return f.fake.AllInRange(f.Start, f.End)
}

func (f *fakeNumberRange) Backward() iter.Seq2[int, int] {
	return f.fake.BackwardInRange(f.Start, f.End)
}
